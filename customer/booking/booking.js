...
document.getElementById('bookBtn').addEventListener('click', () => {
  ...
  // ✅ บันทึกวันที่ + เวลาลง localStorage
  const y = selectedDate.getFullYear();
  const m = String(selectedDate.getMonth() + 1).padStart(2, '0');
  const d = String(selectedDate.getDate()).padStart(2, '0');
  const dateStr   = `${d} ${thMonths[selectedDate.getMonth()]} ${y + 543}`;

  localStorage.setItem('booking_date',     `${y}-${m}-${d}`);
  localStorage.setItem('booking_date_th',  dateStr);
  localStorage.setItem('booking_time',     selectedTime);

  window.location.href = '/customer/booking/booking1.html';
});
