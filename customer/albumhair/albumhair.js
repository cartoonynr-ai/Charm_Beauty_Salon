 const STORAGE_KEY = 'hair';
  ...
  const DEFAULT_TITLES = ['ผู้หญิง', 'ผู้ชาย', 'ทำสี'];

  let slotData = Array.from({ length: GROUPS }, () => Array(SLOTS_PER_GROUP).fill(null).map(() => ({})));
      ...
      for (let s = 0; s < SLOTS_PER_GROUP; s++) {
        ...
        card.innerHTML = `
          ...
          <div class="overlay-hint">
            <span class="text-white text-xs font-semibold">${d.name || 'คลิกเพื่อดูรายละเอียด'}</span>
          </div>`;
  ...
  window.addEventListener('DOMContentLoaded', () => { loadFromAdmin(); renderGrids(); });